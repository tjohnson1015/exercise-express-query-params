 ...
 * @param {'ASC' | 'DESC'} direction the direction to order by ()
 */
function findAll(orderBy = 'id', direction = 'ASC') {
  return sortBy(products, orderBy, direction)
}

/**
 * find one product by its id
 * @param {number} id id of the product
 * @returns {Product | null} the product with the given id or null if not found
 */
 ...
 * @returns {Products[]} the products that match the search criteria
 */
function search(key, value, orderBy = 'id', direction = 'ASC') {
  return sortBy(filterBy(products, key, value), orderBy, direction)
}
